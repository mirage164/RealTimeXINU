...
//======================= Lab 4 ==================================
struct	tmem_entry tmem_tab[NTMEM]; /* time memory table	*/
//================================================================
...
// =================== Lab 4 ====================================
struct	qent	q[NQENT + NTMEM];	/* q table (see queue.c)	q increased by NTMEM size */
int parent_pid[NPROC]; // array to save the parent of each child procees
//================================================================
...
//======================= Lab 4 ==================================
int    tmem_nempty;	/* FALSE if the time memory queue is empty	*/
int    *tmem_top;	/* address of key part of top entry in		*/
				/* the time memory queue if tmem_nempty==TRUE	*/
					
int     tmemq;			/* head of queue of time memory  */
int tmemq_tail;			/* tail of queue of time memory  */
//================================================================
...
LOCAL sysinit()
{
	...
	//======================= Lab 4 ==============================
	for (i=0 ; i<NTMEM ; i++){	/* initialize time memory table	*/
		tmem_tab[i].tmem_index = i;
		tmem_tab[i].tmem_state = TMEMFREE;
	}	
	tmem_tab[0].tmem_state = TMEMUSED; // head is always used
	tmem_tab[1].tmem_state = TMEMUSED; // tail is always used
	tmem_nempty = 0;	/*	initialize to false	*/
	tmemq = newqueue();
	tmemq_tail = tmemq + 1;	/* initialize time memory list head*/
